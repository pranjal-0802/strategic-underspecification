...
\begin{corollary}[Pooling is generically interior]
Because $F$ has support on both sides of the threshold $\kappa^*$ from
Proposition~\ref{prop:firstbest} whenever the population is heterogeneous
($\kappa_L < \kappa^* < \kappa_H$), the single pooling rate $a^{SE}$ is a
compromise strictly between the two first-best corners: neither type receives its
own first-best ask rate. This holds \emph{even when the leader is fully unbiased}
($\mu_A=1,\lambda_A=c_Q$)---pooling loses efficiency purely from being restricted
to one instrument, independent of any misalignment.
\end{corollary}
...
Standard: relax the problem to IC$_L$ and IR$_H$ only (verify ex post the omitted
constraints are slack, routine given single-crossing); use the envelope
representation of type $\kappa_L$'s rent from mimicking $\kappa_H$'s bundle,
$U(m_H,a_H;\kappa_L)-U(m_H,a_H;\kappa_H) = (\kappa_H-\kappa_L)m_H^2/2$; substitute
the binding IR$_H$ and IC$_L$ into the leader's objective to obtain a
``virtual-surplus'' objective in $(m_H,a_H)$ alone, in which the $\kappa_H$ bundle
is charged an extra marginal cost proportional to $f_L/f_H\cdot(\kappa_H-\kappa_L)$
for increasing $m_H$ (since a larger $m_H$ enlarges the rent owed to $\kappa_L$);
this pulls the optimal $a_H$ (and implicitly $m_H$, via the follower's own FOC)
below the leader's unconstrained-per-type preference $a_H^{B}$. We omit the full
explicit substitution; it follows Baron--Myerson/Myerson's virtual-surplus
technique line for line once $\Pi_\kappa$ replaces the classical transfer-based
payoff.
\end{proof}
...
\item \textbf{Model I (pooling)} loses welfare from two additive, independent
sources: (i) the pooling restriction itself, present even when $\mu_A{=}1,
\lambda_A{=}c_Q$ (Corollary 3), and (ii) bias, which shifts $a^{SE}$ further
(Corollary 2). These compound: a biased pooling policy is worse than either
distortion alone would predict.
