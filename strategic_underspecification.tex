...
directly. Loosely, since $\ln q \approx -(1-q)$ for $q$ near 1, the condition is
close to $(k-m)(1-q) \ge 1$, i.e., the expected number of \emph{wrong} attributes
under the current policy is at least one. A numerical audit over
$k\in[5,15]$, $q\in[0.70,0.95]$, and $\kappa\in[0.20,2.00]$ (726 grid points,
evaluated at each point's own optimizing $m^*$) finds the condition holds in only
$10.1\%$ of this range, failing in $100\%$ of points with $\kappa\le0.60$ (users
specify almost everything, so $k-m^*\to0$) and in effectively all points with
$q\ge0.90$ (guessing is accurate enough that $|\ln q|$ is too small for the bound
to bind within $k\le15$). This is precisely the regime of a capable, low-friction
assistant, arguably the empirically relevant case. Two things follow. First, this
is a statement about a \emph{sufficient} condition, not about the conclusion
itself: Corollary~\ref{cor:mono} and Proposition~\ref{prop:bias} could still hold
by a different argument, or could simply fail, outside this region, and the audit
above does not distinguish these possibilities, since it only evaluates whether
the sufficient condition holds, not whether $m^*(\kappa)$ or $m^*(g)$ actually
lose monotonicity when it doesn't. Whether monotonicity survives numerically
outside the $10.1\%$ region is the immediate next check, and until it is done,
Corollary~\ref{cor:mono} and Proposition~\ref{prop:bias} should be read as
formally established only inside that region rather than across the paper's full
