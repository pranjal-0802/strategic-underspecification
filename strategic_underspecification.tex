...
Proposition~\ref{prop:bias} via Topkis's theorem, not a claim about primitives
...
theorem needs to conclude $m^*(g)$ is weakly decreasing --- exactly when
...
$m^*$) finds the corrected condition holds on roughly $31\%$ of this range under
that cost specification, concentrated at the higher-$q$, higher-$\kappa$ corner
(capable assistant, terse-by-cost-averse users) rather than at low $q$; the exact
share is sensitive to the (unspecified in Section~\ref{sec:baseline}) functional
form of $c(m)$ and should be re-run against whatever cost function the empirical
design ultimately commits to. Two things follow regardless of the exact share.
First, this is a statement about a \emph{sufficient} condition, not about the
conclusion itself: Corollary~\ref{cor:mono} and Proposition~\ref{prop:bias} could
still hold by a different argument, or could simply fail, outside this region,
and the audit above does not distinguish these possibilities, since it only
evaluates whether the sufficient condition holds, not whether $m^*(\kappa)$ or
$m^*(g)$ actually lose monotonicity when it doesn't. Second, until that broader
check is done, Corollary~\ref{cor:mono} and Proposition~\ref{prop:bias} should be
read as formally established only inside the region identified above rather than
across the paper's full stated working range.
\end{remark}
