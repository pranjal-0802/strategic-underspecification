...
\begin{remark}[Interpreting Assumption~\ref{ass:reg}]
...
under the current policy is at least one---so it is most likely to hold precisely
when guessing is unreliable or $k-m$ is not small, and can fail once accuracy is
high or few attributes remain unspecified. We do not currently characterize how
restrictive this is over empirically plausible $(k,q)$ ranges (e.g.\ $k\in[5,15]$,
$q\in[0.7,0.95]$); this should be checked numerically rather than assumed to hold
throughout the paper's working range.
\end{remark}
...
\begin{remark}[The two bias channels are not separable, and only one is analyzed]
\label{rmk:mu-lambda}
The model defines two distinct sources of bias, discounting ($\mu_A<1$) and
friction misperception ($\lambda_A\ne c_Q$), but only $\lambda_A$'s comparative
statics are derived above. Expanding $\Pi_\kappa=\mu_A U-(\lambda_A-c_Q)a(k-m)$
using $U$'s own $a(k-m)$ term shows the coefficient the leader actually responds
to on this margin is $\mu_A c_Q+(\lambda_A-c_Q)=\lambda_A+c_Q(\mu_A-1)$, not
$\lambda_A$ alone: a leader that merely discounts downstream welfare
($\mu_A<1$, $\lambda_A=c_Q$) behaves, along this margin, exactly as if it had an
under-asking friction bias, even with no misperception of $\lambda_A$ whatsoever.
$\mu_A$ additionally scales $C_0=\mu_A\Lambda$ elsewhere in $\Pi(a)$, so the two
parameters are not fully redundant, but the overlap above means $\partial
a^{SE}/\partial\mu_A$ should be derived and reported alongside
$\partial a^{SE}/\partial\lambda_A$ before either is treated as an independently
identifiable, testable channel (Section~\ref{sec:predictions}); as written, an
observed low pooling ask rate is consistent with either bias source and the
model does not yet say how field data would distinguish them.
\end{remark}
...
stated without qualification. This qualification applies with particular force to
Corollary~\ref{cor:corner} (pooling is a corner solution) and
Proposition~\ref{prop:distortion} (downward distortion under bias): these are the
paper's two headline results, both are established only under the small-residual
expansion around $q\to1$, and the approximation is least trustworthy precisely
where $q$ is furthest from 1---plausibly the regime in which asking matters most.
Sections~\ref{sec:baseline}--\ref{sec:naive-soph}, by contrast, use the exact
conjunctive form throughout, so the paper currently mixes exactness levels across
its two halves; closing this gap is the single highest-priority next step.
...
circular. An independent handle---for instance, from a training objective or
reward specification that explicitly penalizes turn count---would be needed
before the bias-versus-heterogeneity predictions in Section~\ref{sec:predictions}
can be tested non-circularly. The two-type restriction in Model II avoids
