...
has empirical bite only once bias is already present. Whether the same
non-identification holds on the corner branch, where the relevant comparison is
$\Pi(1)-\Pi(0)$ rather than the interior stationary point, is a separate,
currently open question; the two expressions are not obviously reducible to one
another, and we do not claim ray-invariance across the corner regime without
...
the paper's two headline qualitative claims. It is not a complete check, however:
the exact-model audit tested only a single bias level for the screening problem,
so the further finding (Remark following Proposition~\ref{prop:distortion}) that
$\text{IC}_H$ can bind under sufficiently extreme bias was established only under
the linear-risk approximation and has not yet been confirmed under the exact
conjunctive form; that remains open. Sections~\ref{sec:baseline}--\ref{sec:naive-soph}
use the exact conjunctive form throughout, so with the above audit the paper's
two halves are now substantially, though not completely, reconciled in exactness
level.
