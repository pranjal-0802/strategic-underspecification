...
A burgeoning literature investigates human--AI collaboration through hierarchical bi-level games~\citep{stackelberg1934}, where human task specification sets the boundary conditions for an AI agent's execution. Recent empirical studies in software engineering document an intent ``collaboration gap,'' wherein implicit or under-specified human prompts induce asymmetric repair loops. Our model provides the first formal contract-theoretic microfoundation for this trade-off, characterizing how assistant asking policies and guessing capabilities strategically shape developer specification effort.
...
\begin{remark}[Interpreting Assumption~\ref{ass:reg}, and what remains unchecked]
...
(An earlier draft of this assumption had the inequality reversed, which instead
gives \emph{increasing} differences and hence $m^*(g)$ weakly \emph{increasing}
in that region; a direct numerical optimization of $U$ confirms this reversal
concretely, e.g. at $V{=}10,k{=}5,a{=}0,c(m){=}m^2/2$, $m^*$ rises from
$\approx0.26$ at $g{=}0.5$ to $\approx0.67$ at $g{=}0.9$ --- both points well
inside the old, incorrectly-signed condition's region --- before turning over and
falling as $g\to1$, which is exactly the region the condition as corrected here
identifies.) Loosely, since $\ln q \approx -(1-q)$ for $q$ near 1, the corrected
condition is close to $(k-m)(1-q) \le 1$: the expected number of \emph{wrong}
attributes under the current policy is at most about one. A numerical audit over
...
corrected condition holds on $89.9\%$ (653/726) of this parameter space,
whereas the earlier draft's reversed condition ($\le -1$) held on only $10.1\%$
(73/726). In lower-valuation regimes where specification effort is lower
...
parameter intervals), the follow-up finite-difference audit found no violations
...
Let $R(a,\kappa)=k-m^*(\kappa;a)$ denote the residual unspecified attributes. In the pooling regime, the assistant commits ex ante to an operational ask rate $a$ applied population-wide, without observing or directly contracting on individual specification effort. The assistant's operational payoff on type $\kappa$ evaluates the residual attributes: downstream success $\mu_A V$ minus expected resolution and dialogue friction costs $C(a)R(a,\kappa)$, where $C(a)=\mu_A\Lambda(1-a) + a\lambda_A = \mu_A\Lambda + a(\lambda_A - \mu_A\Lambda)$.\footnote{User specification effort $c(m)$ is sunk from the assistant's operational perspective. Even if the leader additionally internalized the follower's private effort cost directly via the total welfare objective $\mathbb{E}[U(m^*(\kappa;a),a;\kappa)]$, in the unbiased case ($\mu_A=1, \lambda_A=c_Q$) direct differentiation gives $d^2\mathbb{E}[U]/da^2 = (\Lambda-c_Q)^2 \mathbb{E}[1/\kappa] \ge 0$, which remains strictly convex on $[0,1]$. Thus, Corollary~\ref{cor:corner}'s central qualitative result---that unbiased pooling is always a corner solution rather than an interior compromise---survives identically under both specifications.} Averaging over $F$ with $\bar R(a) = \bar R_0 + a\bar\gamma$ (where $\bar R_0=k-\mathbb{E}[\beta]$, $\bar\gamma=\mathbb{E}[\gamma]$), $\Pi(a)$ is quadratic in $a$:
\[
...
Differentiating the closed form in Proposition~\ref{prop:aSE} directly with
respect to $\lambda_A$ (holding $\mu_A,\Lambda,\bar R_0,\bar\gamma$ fixed) gives,
on the interior branch $\Delta\bar\gamma>0$,
\[
\frac{\partial a^{SE}}{\partial \lambda_A} \;=\; \frac{\mu_A\Lambda}{2(\mu_A\Lambda-\lambda_A)^2}
\;>\;0 \qquad \text{whenever } \mu_A,\Lambda>0 \text{ and } \lambda_A\ne\mu_A\Lambda,
\]
so $a^{SE}$ is strictly \emph{increasing} in $\lambda_A$ throughout the interior
branch --- the opposite direction from an earlier draft of this corollary, which
claimed the sign was that of $2\bar\gamma a^{SE}-\bar R_0$ and concluded
$a^{SE}$ was decreasing; that claim does not match implicit differentiation of
the leader's own first-order condition and is superseded by the derivative
above. This comparative static should also be read narrowly: by
Corollary~\ref{cor:corner}, the interior stationary point is a
\emph{minimizer} of $\Pi(a)$ whenever $\Delta\bar\gamma\le0$, which includes the
entire unbiased case, so the interior branch on which this corollary is stated
is already a departure from the unbiased benchmark, and the corollary describes
how a (locally optimal, on that branch) interior rate moves with $\lambda_A$,
not how the \emph{globally} optimal pooling rate moves in general --- for that,
see the corner comparison in Corollary~\ref{cor:corner-ray} below, which holds
unconditionally.
\end{corollary}

\begin{corollary}[Non-identification of the two bias channels in the interior regime]
\label{cor:mu-lambda}
On the interior branch ($\Delta\bar\gamma>0$), substituting $C_0=\mu_A\Lambda$ and
$\Delta=\lambda_A-\mu_A\Lambda$ into Proposition~\ref{prop:aSE} gives
\[
a^{SE} \;=\; -\frac{\bar R_0}{2\bar\gamma} \;-\; \frac{1}{2}
\left[\frac{1}{\lambda_A/(\mu_A\Lambda)-1}\right],
\]
so $a^{SE}$ depends on the two bias parameters strictly through the scalar ratio
$\lambda_A/\mu_A$ (holding $\Lambda$, the model primitive, fixed), not through
$\lambda_A$ and $\mu_A$ separately. Consequently
$\partial a^{SE}/\partial\mu_A = -(\lambda_A/\mu_A)\cdot\partial
a^{SE}/\partial\lambda_A$ exactly, and any two parameter pairs on the same ray
$\lambda_A/\mu_A=\text{const}$ produce the identical pooling ask rate. A leader
that merely discounts downstream welfare ($\mu_A<1$, $\lambda_A=c_Q$) is
therefore observationally indistinguishable, from $a^{SE}$ alone, from a leader
with an under-asking friction bias at the corresponding $\lambda_A$ on the same
ray: neither Corollary~2 nor this result identifies which channel is responsible
for a low observed ask rate, and a field test would need an instrument
independent of $a^{SE}$ itself (e.g.\ a training-objective measure of
$\lambda_A$) to separate them.
\end{corollary}

\begin{remark}[Scope of Corollary~\ref{cor:mu-lambda}]
The derivation above, and its numerical confirmation, are both restricted to the
interior branch $\Delta\bar\gamma>0$. By Corollary~\ref{cor:corner} below, the
\emph{unbiased} pooling optimum is always a corner, so the ray-invariance result
has empirical bite only once bias is already present.
\end{remark}

\begin{corollary}[Ray-invariance extends to the corner regime]\label{cor:corner-ray}
On the corner branch ($\Delta\bar\gamma\le 0$), the relevant comparison is
$\Pi(1)-\Pi(0)$ rather than the interior stationary point. Substituting
$C_0=\mu_A\Lambda$ and $\Delta=\lambda_A-\mu_A\Lambda$ into that difference gives
\[
\Pi(1)-\Pi(0) \;=\; \mu_A\Big[\Lambda\bar R_0 - \tfrac{\lambda_A}{\mu_A}\big(\bar
R_0+\bar\gamma\big)\Big],
\]
so $\mu_A>0$ again factors out completely and the sign depends only on the
scalar ratio $\lambda_A/\mu_A$, exactly as on the interior branch. The critical
ratio is
\[
\rho^{*} \;=\; \frac{\Lambda\bar R_0}{\bar R_0+\bar\gamma},
\]
with $a^{SE}=1$ for $\lambda_A/\mu_A<\rho^{*}$ and $a^{SE}=0$ for
$\lambda_A/\mu_A>\rho^{*}$. Ray-invariance therefore holds unconditionally
across the entire parameter space, interior and corner regimes alike,
resolving the open question raised above. A numerical sweep of 11 rays across
10 values of $\mu_A\in[0.1,1.0]$ (110 evaluations) confirms the analytical
threshold exactly and finds zero boundary-crossing violations.
\end{corollary}
...
A numerical audit re-deriving both the unbiased pooling optimum and the biased
screening menu directly under the exact conjunctive payoff $q(a,g)^{k-m}$
(independently of the linear-risk closed forms, via direct grid search and
constrained optimization) finds the corner-solution property of
Corollary~\ref{cor:corner} surviving in every tested configuration across
$k\in[3,20]$, $g\in[0.50,0.95]$, and the downward-distortion direction of
Proposition~\ref{prop:distortion} surviving in the large majority of
configurations, with the exceptions concentrated at small $k$ where specification
effort saturates at $m=k$ and the asking wedge collapses. This is reassuring for
the paper's two headline qualitative claims. The initial exact-model audit tested
only a single bias level for the screening problem, leaving open whether the
further finding (Remark following Proposition~\ref{prop:distortion}) that
$\text{IC}_H$ can bind under sufficiently extreme bias was an artifact of the
linear-risk approximation. A follow-up sweep of the full four-variable
constrained menu problem, solved directly under the exact conjunctive payoff
across $\lambda_A\in[c_Q,10c_Q]$ and $\mu_A\in[0.1,1.0]$ (100 grid points),
resolves this: $\text{IC}_H$ binds alongside $\text{IC}_L$ in 89 of the 100
points, reproducing the same three-regime structure (no binding constraint,
$\text{IC}_L$ alone, $\text{IC}_L+\text{IC}_H$ together) found under the
linear-risk approximation, with only the boundary between the moderate- and
extreme-bias regimes shifting slightly (from $\lambda_A\approx6.0$ to
$\lambda_A\approx8.0$ at $\mu_A=1.0$). The extreme-bias reversal is therefore an
intrinsic feature of the screening contract space, not an artifact of the
linear-risk approximation. Sections~\ref{sec:baseline}--\ref{sec:naive-soph}
use the exact conjunctive form throughout, so with the above audits the paper's
two halves are now substantially reconciled in exactness level, modulo the
small-$k$ saturation caveat noted above.
