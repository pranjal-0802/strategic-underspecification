...
def run_verification(output_dir: str = "outputs") -> Dict[str, Any]:
    ...
    ax1.set_title(r"Corollary 2: Bias Direction on Interior Branch ($\Delta\bar\gamma > 0$)" + "\n" + r"$\partial a^{SE}/\partial\lambda_A > 0$ strictly holds")
    ...
    ax2.plot(df["lambda_A"], df["analytical_derivative"], "--", color="crimson", linewidth=1.5, label=r"Analytical $\frac{\mu_A\Lambda}{2(\mu_A\Lambda - \lambda_A)^2}$")
...
### Findings:
1. **Strictly Interior Regime:** Swept $\\lambda_A \\in [{df['lambda_A'].min():.2f}, {df['lambda_A'].max():.2f}]$ across {len(df)} points. All points satisfy $\\Delta\\bar\\gamma > 0$ (SOC holds) with $a^{{SE}} \\in [{df['a_SE'].min():.3f}, {df['a_SE'].max():.3f}]$ strictly away from boundaries.
