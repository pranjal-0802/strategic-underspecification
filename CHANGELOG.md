...
## v4.0.0 — Final Preprint Synchronization (Current)

- **One Canonical Manuscript:** Consolidated the paper into `paper/strategic_underspecification.tex` (and compiled preprint `paper/strategic_underspecification.pdf`), removing all versioned suffixes (`_v1`, `_v2`, `_v3`, `_v4`). Added complete BibTeX bibliography in `paper/references.bib` and high-resolution figures in `paper/figures/`.
- **Resolution of Critical Assumption 1 Discrepancy:**
  - Traced the origin of the 10.1% vs 31% discrepancy to both mathematical direction and task-value scaling:
    - The legacy 10.1% artifact was generated by the pre-fix *reversed* condition $(k - m^*)\ln q \le -1$ at $V = 100.0$ (73 / 726 points).
    - Under the corrected condition $(k - m^*)\ln q \ge -1$ required for Topkis decreasing differences, the condition holds in **89.9%** (653 / 726 points) on the canonical 726-point grid ($V = 100.0$).
    - The ~31% draft statement was estimated under lower task valuations ($V \approx 7.5 - 10$, where $V=10$ yields 34.3% and $V=8$ yields 31.7%), where higher $\kappa/V$ induces users to specify fewer attributes ($m^*$ lower, $k-m^*$ higher).
    - Added an exact regression test in `tests/test_assumption1.py` locking in the canonical 89.94% (653/726) result so it cannot silently drift.
  - Crucially, the comprehensive finite-difference audit across 2,552 intervals (`verify_monotonicity_survival.py`) confirms empirical monotonicity of $m^*(g)$ survives in **100% of tested parameter intervals** outside the sufficient condition ($2,162 / 2,162$).
- **Scientific Precision & Human-Study Clarification:**
  - Explicitly distinguished analytical theorems, numerical grid checks, LLM experiments, and proposed human-study designs.
  - Clarified in both the paper and documentation that the computational evaluation consists of analytical verification, numerical experiments, and LLM-based simulation experiments; no human-subject study is reported.
  - Replaced imprecise phrases like "confirms 100% agreement" with "agrees across all tested parameter configurations".
- **Regenerated Verification Artifacts:**
  - Executed all 14 mathematical checks via `run_all_math_checks.py`, regenerating all CSVs, Markdown summaries, and PNG figures in `outputs/` and `paper/figures/`.
- **Test Suite Expansion:**
  - 46 unit, regression, and symbolic tests (100% passing) with zero failures.
- **Security & Metadata Audit:**
  - Verified no API keys, secrets, or tokens are committed; added `.env`, `.env.*`, and `*.db` to `.gitignore`.
  - Updated `CITATION.cff`, `pyproject.toml`, and `underspec_sim/__init__.py` to version `4.0.0` with valid CFF metadata.

---

## Historical Releases

### Mathematical Resync & Downstream Pipeline Validation (v3 Synchronization)

Following the updates to the foundational paper [*Strategic Under-Specification: A Stackelberg Game Between User and Assistant*](paper/strategic_underspecification.tex), this release synchronized the theoretical derivations, updated the pooling equilibrium solver, extended the constraint-activity audit for screening under extreme bias, and validated all downstream numerical pipelines.
