...
### Key Results:
- **Dominance Corollary Verified:** Across all 25 2D grid points, Model II payoff weakly exceeds Model I (Min gap: `-0.000000`, Mean gap: `0.5380`, Max gap: `3.3333`).
- **Audit Against Corrected Solver:**
  ...
  - In this 2D grid ($k=10, L=10, c_Q=2$), pooling is at a corner in 100% of grid points (`pooling_regime: {'corner': 15, 'interior': 10}`).
  - Model I payoff reaches a maximum of `85.56` and minimum of `77.83`.
...
### Decomposition:
1. **Instrument Restriction Loss (Zero Bias):** At $\\lambda_A = c_Q$, Model II dominates Model I with a positive gap (e.g., gap = `3.33` at $\\Delta\\kappa=0.30$) purely because pooling cannot offer type-contingent ask rates.
